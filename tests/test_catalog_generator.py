...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"Unable to load catalog generator from {MODULE_PATH}")
...
class CatalogGeneratorTests(unittest.TestCase):
    def test_build_catalog_lists_audio_files_with_stable_relative_paths(self) -> None:
        with tempfile.TemporaryDirectory() as temp_directory:
            ...
            (repository_root / "music" / "album" / "anthem.MP3").write_bytes(
                b"upper-mp3"
            )
            (repository_root / "music" / "album" / "beat.WAV").write_bytes(
                b"upper-wav"
            )
            (repository_root / "music" / "album" / "track.wav").write_bytes(
                b"wav"
            )
            (repository_root / "music" / "album" / "track.mp3").write_bytes(
                b"mp3"
            )
            (repository_root / "music" / "album" / "notes.txt").write_text(
                "ignore",
                encoding="utf-8",
            )
            ...
            (
                repository_root / ".github" / "workflows" / "ignored.mp3"
            ).write_bytes(b"github")
            (
                repository_root / "node_modules" / "demo" / "ignored.mp3"
            ).write_bytes(b"node")
        ...
        self.assertEqual(
            ...
            [
                {
                    "filename": "anthem.MP3",
                    "path": "music/album/anthem.MP3",
                },
                {
                    "filename": "beat.WAV",
                    "path": "music/album/beat.WAV",
                },
                {
                    "filename": "track.mp3",
